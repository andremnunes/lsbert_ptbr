...
def substitution_ranking(source_word, source_context, substitution_selection, fasttext_dico, fasttext_emb, word_count, ssPPDB, tokenizer, maskedLM, lables):
    ...
    dict_ranking_candidatas = {ss[i]: all_ranks[i] for i in range(len(ss))}

    list_ranking_candidatas = sorted(dict_ranking_candidatas.items(), key=lambda x:x[1])
    dict_ranking_candidatas = dict(list_ranking_candidatas)

    list_candidate_words = list(dict_ranking_candidatas.keys())

    return pre_word, list_candidate_words
...
def main():
    ...
    parser.add_argument("--bert_model", default=None, type=str, required=True,
                        ...
                        "bert-large-uncased, bert-base-cased, bert-large-cased, bert-base-multilingual-uncased, neuralmind/bert-base-portuguese-cased, "
    ...
    #model = BertForPreTraining.from_pretrained('neuralmind/bert-base-portuguese-cased')
    ...
    output_prediction_file = open(args.output_prediction_file,"a+")
    output_log_file = open(args.output_log_file,"a+")
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for i in range(eval_size):
            ...
            pos_tags = pos_tag_pt.tag(pre_tokens)
            tags_exclusao = ['ART', 'NPROP', 'NUM', 'PREP', 'PROPESS', 'PRO-KS-REL', 'PROSUB', 'CUR', '|EST','|AP','|DAD','|TEL','|DAT','|HOR','|[ |]']
            ignore_word_tag = [word for (word,tag) in pos_tags if tag in tags_exclusao]

            # removing stop words from tokenized
            ignore_word_stopwords = [word for word in pre_tokens if word in stopwords]

            # removing punctuation from tokenized
            ignore_word_ponctuation = [word for word in pre_tokens if word in string.punctuation]

            list_all_ignore = sorted(list(set(ignore_word_tag + ignore_word_stopwords + ignore_word_ponctuation)))

            pre_tokens = [word for word in pre_tokens if (word not in list_all_ignore or len(word) >= 3)]
            ...
            pre_word, list_candidate_words = substitution_ranking(mask_words[i], mask_context, cgBERT, fasttext_dico, fasttext_emb,word_count,cgPPDB,tokenizer,model,mask_labels[i])


            substitution_words.append(pre_word)
          
            # Prepara arquivo saí­da
            output_log_file.write('\n')
            output_log_file.write('Frase {} rankings: '.format(i))
            output_log_file.write(str(eval_examples[i]))
            output_log_file.write('\n')

            output_log_file.write('Palavra complexa: ')           
            output_log_file.write(str(mask_words[i]))
            output_log_file.write('\n')

            output_log_file.write('Palavras candidatas BERT: ')
            for w in CGBERT[i]:
              output_log_file.write(str(w))
              output_log_file.write('\t')
            output_log_file.write('\n')

            output_log_file.write('Palavras candidatas Ranqueadas: ')
            for w in list_candidate_words:
              output_log_file.write(str(w))
              output_log_file.write('\t')
            output_log_file.write('\n')

            output_log_file.write('Substituta: ')
            output_log_file.write(str(pre_word))
            output_log_file.write('\n')

            print('Palavra complexa:', mask_words[i])
            print('Palavras candidatas BERT:', CGBERT[i])
            print('Palavras candidatas Ranqueadas:', list_candidate_words)
            print('Substituta:', pre_word)
            print('\n')

            # Prepara arquivo saí­da
            output_prediction_file.write(str(eval_examples[i]))
            output_prediction_file.write('\t')
            output_prediction_file.write(str(mask_words[i]))
            output_prediction_file.write('\t')
            for w in list_candidate_words:
              output_prediction_file.write(str(w))
              output_prediction_file.write('\t')
            output_prediction_file.write('\n')
