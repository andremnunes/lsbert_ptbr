...
class Ranker:
    ...
    def __init__(self):
        ...
        self.ps = SnowballStemmer("portuguese")
...
def convert_sentence_to_token(sentence, seq_length, tokenizer):
    ...
    nltk_sent = nltk.word_tokenize(sentence.lower(), language='portuguese')
...
def recursive_simplification( model, tokenizer, ranker, sentence, tokens, positions, max_seq_length, tokenized, threshold = 0.5, num_selections=10, ignore_list = []):
    ...
    if (len(sentence_object.complex_words) > 0): #if there are complex words in the sentence
        ...
        print('sentence_object.complex_words',sentence_object.complex_words)
        ...
        #create word object 
        print('original word:', sentence_object.tokenized[index])
        ...
        #print('Candidatos gerados pelo BERT ')
        #print(cgBERT[:10])
...
def simplified_sentence(one_sent, model, tokenizer, ranker, max_seq_length=350, threshold=0.5, num_selections=5 ):   
    
    nltk_sent = nltk.word_tokenize(one_sent, language='portuguese')
...
def main():
    ...
    # Teste com uma sentença
    #one_sent = "ao prever a distribuição gratuita de uniforme para alunos da rede pública um projeto em discussão no congresso institui uma controvérsia em escolas prefeituras e secretarias de educação"
    one_sent = "O exame ajuda a elucidar o dignóstico"
    ...
    print("Sentença Original",one_sent)
    print("Sentença Simplificada", simple_sent)
    ...
    with open(args.eval_dir, "r") as reader:
        ...
        #while True:
            ...
            one_sent = one_sent + str("\t") + "FIM"

            sentence,words = one_sent.strip().split('\t',1)
            one_sent = sentence
            ...
            simple_sent = simplified_sentence(one_sent, model, tokenizer, ranker, args.max_seq_length, threshold=0.7, num_selections=args.num_selections )
