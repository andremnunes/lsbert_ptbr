...
class Sentence:
	...
	def __init__(self, tokenized, threshold, ignore_list):
		...
		self.pos_tags = pos_tag_pt.tag(self.tokenized)
		if ignore_list == []:
			tags_exclusao = ['ART', 'NPROP', 'NUM', 'PREP', 'PROPESS', 'PRO-KS-REL', 'PROSUB', 'CUR', '|EST','|AP','|DAD','|TEL','|DAT','|HOR','|[ |]']     
			self.ignore_index = [c for (a,b),(c,d) in zip(self.pos_tags, self.indexes) if b in tags_exclusao]
		else:
			self.ignore_index = ignore_list

		# removing stop words from tokenized
		tokenized_stopwords = [c for (a,b),(c,d) in zip(self.pos_tags, self.indexes) if a in stopwords]

    # removing punctuation from tokenized
		tokenized_ponctuation = [c for (a,b),(c,d) in zip(self.pos_tags, self.indexes) if a in string.punctuation]

		self.ignore_index = sorted(list(set(self.ignore_index + tokenized_ponctuation + tokenized_stopwords)))
...
class Word:
	...
	def get_synonyms(self):
		spacy_module = spacy.load('pt_core_web_sm')
